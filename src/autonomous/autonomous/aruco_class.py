...
class ArucoClass:
    def __init__(self):
        ...
        self.DISTORSION_COEFFS = [2.9425814151763916, 0.7698521018028259, -3.687290518428199e-05, 0.00017509849567431957,
        ...
        self.DISTORSION_COEFFS = np.array(self.DISTORSION_COEFFS, dtype=np.float32)
    ...
    def final_aruco_info(self, frame):
        ...
        if ids is not None:
            for i, marker_ids in enumerate(ids):
                ...
                distance_estimate = self.calculateDistance(corners[0], self.K, self.DISTORSION_COEFFS)
    ...
    def drive_logic(self, frame, localization, imu, time_now):
        ...
        if no_tag_found_yet:
            ...
            if not self.ended_reorientation:
                if not self.oriented_to_re_path:
                    ...
                    if (initial_rad_diff < 0 and current_rad_diff < 0) or (
                            initial_rad_diff > 0 and current_rad_diff > 0):
                        lin_x = 1.0
                        lin_y = 0.0
                        ang_pos = 0.0
                        ang_neg = 0.0

...
class DriveLogic(Node):
    def __init__(self):
        ...
        self.bottle_sub = self.create_subscription(Detection, 'detection_msg', self.bottle_cb, 10)
    ...
    def drive_callback(self):
        ...
        # imu = self.imu_msg
        if self.bottle_msg is None:
            self.get_logger().info("Waiting for bottle detection...")
            return
        conf = self.bottle_msg.conf
    
        dist = self.calculate_bottle_distance(self.bottle_msg.y1, self.bottle_msg.y2)

        x = (self.bottle_msg.x1 + self.bottle_msg.x2) / 2.0
        
        is_visible = conf > 0.5 

        no_tag_found_yet = (not is_visible and self.ended_search_drive == False)

        detecting_a_tag = is_visible

        tag_found_but_not_being_detected = (not is_visible and self.ended_search_drive == True)
        ...
        self.get_logger().info(f"bottle: {self.bottle_msg}")
        ...
        if no_tag_found_yet:
            ...
            linear_velocity = 1.5
            linear_vel, angular_vel = go_to_tag(x, dist, linear_velocity)

            self.full_search_done = True
            self.ended_search_drive = True
            self.previous_offset = x
            self.previous_dis = dist

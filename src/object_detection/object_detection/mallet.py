...
class MalletDetector(Node):
    def __init__(self):
        # --- Initialize publisher ---
        super().__init__('mallet_detector')
        self.publisher_ = self.create_publisher(Detection, 'mallet_detections', 10)
        
        # --- Find the model path ---
        model_path = os.path.join(get_package_share_directory('object_detection'), 'resource', 'mallet.blob')

...
class CANSubscriber(Node):
    ...
    def __init__(self):
        ...
        self.temp_fet_pub = self.create_publisher(Float32, 'temp_fet', max_queue)
        self.temp_motor_pub = self.create_publisher(Float32, 'temp_motor', max_queue)
        self.current_in_pub = self.create_publisher(Float32, 'current_in', max_queue)
        self.pid_pos_pub = self.create_publisher(Float32, 'pid_position', max_queue)
...
def receive_canbus(num_messages: int, infty: bool = False):
    ...
    def receive_canbus(self, num_messages: int, infty: bool = False):
        ...
        """
        channel = 'can0'
        # NOTE: Opening and closing the bus this frequently is inefficient.
        # A better long-term solution is to have a persistent bus object and reader thread.
        with can.Bus(channel=channel, interface='socketcan', receive_own_messages=False) as bus:
        ...
        for msg in bus:
            ...
            self.get_logger().debug(f"Received CAN message. Command id {command_id} with vesc id {vesc_id}")
            ...
            match command_id:
                case 16:
                    ...
                    temp_fet_msg = Float32()
                    temp_fet_msg.data = temp_fet_degc
                    self.temp_fet_pub.publish(temp_fet_msg)
                    ...
                    temp_motor_msg = Float32()
                    temp_motor_msg.data = temp_motor_degc
                    self.temp_motor_pub.publish(temp_motor_msg)
                    ...
                    current_in_msg = Float32()
                    current_in_msg.data = current_amps
                    self.current_in_pub.publish(current_in_msg)
                    ...
                    pid_pos_msg = Float32()
                    pid_pos_msg.data = pid_pos_deg
                    self.pid_pos_pub.publish(pid_pos_msg)
...
# NOTE: The functions below are kept separate from the class for modularity,
# but they could also be methods of the class if that fits the design better.
# The `receive_canbus` function was moved into the class to access publishers.

# Bug fix: The original code had a bug where multiple publishers were assigned to the same variable.
# I have fixed this in the diff above by giving them unique names (e.g., `temp_fet_pub`).
